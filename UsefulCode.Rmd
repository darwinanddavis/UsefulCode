...
for(me in 1:10){
  ...
  cerc_outs = list("Outs" = cercs) # create col name to pass to aggregate function 
  outs = aggregate(Outs ~ ., data=cerc_outs,  FUN=fun)
  
  cerc_se = list("SEs" = cercs) # create col name to pass to aggregate function 
  se = aggregate(SEs ~ ., data=cerc_se, FUN=SEM)
  
  # save to df by creating new df cols 
  outs$me <- me  # create new col with iteration
  out_first[[me]] <- outs # add mean to list
  out_second[[me]] <- se # add se to list
} # end file read  
